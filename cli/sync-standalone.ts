import { VaultAdapter, VaultFile, normalizePath } from "../src/vault-adapter";
import { GranolaApiClient } from "../src/api";
import { FigmaClient } from "../src/figma";
import { LinearClient } from "../src/linear";
...
import { HubSpotClient } from "../src/hubspot";
...
/** Extract basename (filename without extension) from a VaultFile path. */
function vaultBasename(f: VaultFile): string {
...
export class SyncEngine {
  ...
  async sync(): Promise<SyncResult> {
    ...
    for (const doc of docs) {
      try {
        ...
        if (existingFile instanceof VaultFile) {
  ...
  private async syncLinearIssues(): Promise<number> {
    ...
    for (const issue of issues) {
      ...
      if (existingFile instanceof VaultFile) {
        const existing = await this.vault.read(existingFile);
        const existingUpdated =
  ...
  private async syncSlackMessages(client: SlackClient): Promise<number> {
    ...
    const allVaultFiles = this.vault.getMarkdownFiles();
    ...
    for (const channel of channels) {
      try {
        ...
        for (const msg of pins) {
          ...
          if (this.vault.getAbstractFileByPath(filePath)) continue;
  ...
  private async writeGoogleDriveAttachment(
    ...
    const existingFile = this.vault.getAbstractFileByPath(attachmentPath);

    if (existingFile instanceof VaultFile) {
      await this.vault.modifyBinary(existingFile, payload);
    } else {
      await this.vault.createBinary(attachmentPath, payload);
    }
        ...
        if (settings.aiEnabled && settings.claudeApiKey) {
          try {
            ...
            const customerMeetings = meetingFiles.filter((f) =>
              vaultBasename(f).toLowerCase().includes(customerLower),
            );
            const excerpts: string[] = [];
            for (const mf of customerMeetings.slice(0, 5)) {
              const raw = await this.vault.read(mf);
        ...
        const health = calculateHealthScore(
          ...
          sentimentScore,
          {
            openDeals: hubspotDeals.filter((f) =>
              vaultBasename(f).toLowerCase().includes(customer.toLowerCase()),
            ).length,
